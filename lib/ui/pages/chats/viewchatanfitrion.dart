...
class ViewChatAnfitrion extends StatefulWidget {
  final MensajesHotel chat;
  const ViewChatAnfitrion({super.key, required this.chat});

  @override
  State<ViewChatAnfitrion> createState() => _ViewChatAnfitrionState();
}

class _ViewChatAnfitrionState extends State<ViewChatAnfitrion> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: buildAppBar(widget.chat.nombreCliente),
      body: Container(
  ...
  AppBar buildAppBar(String nombreCliente) {
    return AppBar(
      ...
      title: Row(
        children: [
          ...
          CircleAvatar(
            ...
            child: Text(
              widget.chat.nombreCliente[0].toUpperCase(),
              style: TextStyle(
                  fontSize: 24,
          ...
          Expanded(
            child: Text(
              nombreCliente,
              overflow:
                  nombreCliente.length > 10 ? TextOverflow.ellipsis : null,
              maxLines: 1,
    ...
    return Container(
      child: StreamBuilder(
        stream: FirebaseFirestore.instance
            ...
            .where('id',
                isEqualTo:
                    "${widget.chat.idReserva}") // Filtro por la reserva actual
            .snapshots(),
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          if (snapshot.hasData && snapshot.data!.docs.isNotEmpty) {
            ...
            sortedDocs.sort((a, b) {
              // Obtener los valores de "position" de cada documento
              int positionA = a['position'];
              int positionB = b['position'];

              // Comparar los valores de "position" para ordenar
              return positionA.compareTo(positionB);
            });

            PeticionesChats.ActualizarVistoReserva("${widget.chat.idReserva}");
